...
class TodoProvider extends ChangeNotifier{
  List<Todo> _posts = [];
  List<Todo> get posts => _posts;

  Future<List<Todo>> fetchPosts(String title, bool completed, int userId) async{

    try{

      var response= await http.get(Uri.parse("https://jsonplaceholder.typicode.com/todos"),headers: {});

      if(response.statusCode == 200){
        List<dynamic> jsonData= jsonDecode(response.body);
        List<Todo> posts= jsonData.map((json)=>Todo.fromJson(json)).toList();
        return posts;
      }
      else{
  ...
  Future<Todo> createPost(Todo post, BuildContext context ) async {
    try {
      ...
      if (postResponse.statusCode == 201) {
        final newPost = Todo.fromJson(jsonDecode(postResponse.body));
  ...
  Future<Todo> updatePost(Todo post,BuildContext context) async {
    try {
      var putResponse = await http.put(
        Uri.parse("https://jsonplaceholder.typicode.com/todos/${post.id}"),
        body: jsonEncode(post.toJson()),
        headers: {'Content-Type': 'application/json'},
      );
      ...
      if (putResponse.statusCode == 200) {
        final replacedPost = Todo.fromJson(jsonDecode(putResponse.body));
