...
class AlbumsProvider extends ChangeNotifier{
  List<Albums> _posts = [];
  List<Albums> get posts => _posts;

  Future<List<Albums>> fetchPosts(String title, int UserId) async{

    try{

      var response= await http.get(Uri.parse("https://jsonplaceholder.typicode.com/albums"),headers: {});

      if(response.statusCode == 200){
        List<dynamic> jsonData= jsonDecode(response.body);
        List<Albums> posts= jsonData.map((json)=>Albums.fromJson(json)).toList();
        return posts;
      }
      else{
  ...
  Future<Albums> createPost(Albums post, BuildContext context ) async {
    try {
      ...
      if (postResponse.statusCode == 201) {
        final newPost = Albums.fromJson(jsonDecode(postResponse.body));
  ...
  Future<Albums> updatePost(Albums post,BuildContext context) async {
    try {
      var putResponse = await http.put(
        Uri.parse("https://jsonplaceholder.typicode.com/albums/${post.id}"),
        body: jsonEncode(post.toJson()),
        headers: {'Content-Type': 'application/json'},
      );
      ...
      if (putResponse.statusCode == 200) {
        final replacedPost = Albums.fromJson(jsonDecode(putResponse.body));
